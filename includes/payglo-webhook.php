...
require_once __DIR__ . '/../payglo-payment.php';

class Payglo_Webhook
{
    ...
    {
        $this->payglo = new WC_Payglo_Payment();
    ...
    public function process()
    {
        ...
        $enabled = $this->payglo->get_option('enable_webhook');
        ...
        if (($enabled === 'yes') and (empty($data['event_type']) === false)) {
            ...
            $this->payglo->post_log("WC_WEBHOOK_RECEIVED", $data['event_type'], $payment_id);
    ...
    protected function paymentAuthorized(array $data)
    {
        ...
        if ($order) {
            ...
            if ($order->status !== 'processing') {
                $this->payglo->post_log("WC_ORDER_PLACED", null, $payment_id);
            }
            $order->payment_complete($payment_id);
            $order->add_order_note('Payment successful via ' . $payment_method . ' (Payglo Payment ID: ' . $payment_id . ') (via Payglo Webhook)');
            if ($data['object']['capture_method'] === "automatic") {
                $order->update_meta_data('payment_captured', 'yes');
            }
            $this->payglo->post_log("WC_PAYMENT_SUCCEEDED_WEBHOOK", null, $payment_id);
    ...
    protected function paymentHandle(array $data, $status)
    {
        ...
        if ($order) {
            ...
            if ($status == 'processing') {
                $order->update_status($this->payglo->processing_payment_order_status);
            } else {
                $order->update_status('pending');
            }
            $order->add_order_note('Payment ' . $status . ' via ' . $payment_method . ' (Payglo Payment ID: ' . $payment_id . ') (via Payglo Webhook)');
            $this->payglo->post_log("WC_PAYMENT_" . str_replace(" ", "_", strtoupper($status)) . "_WEBHOOK", null, $payment_id);
        }
    ...
    //         $order_id = $data['content']['object']['metadata']['order_num'];
    //     } else {
    //         $payment_intent = $this->payglo->retrieve_payment_intent($payment_id);
    ...
    //         $targetRefund = null;
    //         $order->add_order_note('Refund Webhook data: ' . $refund_num . ' ' . $order_id . ' ' . $payment_id . ' ' . $refund_id);
    //         if (isset($refund_num)) {
    ...
    //             }
    //         }
    //         if (!isset($targetRefund)) { // Refund initiated via Payglo Dashboard
    ...
    //         }
    //         $order->add_order_note('Refund Successful (Payglo Refund ID: ' . $refund_id . ') (via Payglo Webhook)');
    ...
    {
        $webhook_key = $this->payglo->get_option('webhook_secret_key');
